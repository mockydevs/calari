...
import { PROVIDER_LABELS } from "@/lib/api-keys";
...
export default async function AiSettingsPage() {
  ...
  const keys = await prisma.aiApiKey.findMany({
    ...
    orderBy: [{ provider: "asc" }, { active: "desc" }, { updatedAt: "desc" }],
  });

  const keysByProvider = new Map<AIProvider, typeof keys>();
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_390px]">
        <div className="space-y-4">
          {PROVIDERS.map((provider) => {
            ...
            const activeKey = providerKeys.find((key) => key.active);
            return (
              <section
                key={provider}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        {activeKey ? `Active key: ${activeKey.keyPreview}` : "No active key"}
                      </p>
                    </div>
                  </div>
                  {activeKey ? (
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[680px] text-sm">
                      <thead className="border-b border-slate-100 bg-white">
                        <tr>
                          {["Label", "Preview", "Created", "Updated", "Actions"].map((heading) => (
                            <th
                              key={heading}
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {providerKeys.map((key) => (
                          <tr key={key.id} className="transition-colors hover:bg-cyan-50/30">
                            ...
                            <td className="px-5 py-3.5 font-mono text-xs text-slate-600">{key.keyPreview}</td>
                            <td className="px-5 py-3.5 text-slate-500">
                              {key.createdBy.name} - {formatDate(key.createdAt)}
                            </td>
                            <td className="px-5 py-3.5 text-slate-500">
                              {key.updatedBy?.name ?? key.createdBy.name} - {formatDate(key.updatedAt)}
                            </td>
                            <td className="px-5 py-3.5">
                              <div className="flex items-center gap-2">
                                {!key.active && (
              ...
              <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-slate-200 bg-slate-50/70 px-3 py-3 text-sm">
                ...
                <span>
                  ...
                  <span className="block text-xs text-slate-500">
                    Activating this key disables other active keys for the same provider.
                  </span>
                </span>
              </label>
